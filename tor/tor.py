...
class TorControl(object):
    ...
    def initialize(self, proxy_port):
        try:
            TorSessionSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            TorSessionSocket.connect((self.proxy_host, proxy_port))
            proxy_session = TorSessionSocket
        except socket.error:
            raise RuntimeError(f"Error Connecting To Tor Server @ {self.proxy_host}:{proxy_port}")
        req1 = b"\x05\x01\x00"  # SOCKS5, ONE AUTH, NO AUTHENTICATION
        proxy_session.send(req1)
        resp1 = proxy_session.recv(2)
        if resp1[1] is not 0:  # "\x00"
            raise RuntimeError
        return True
    ...
    def authenticate(self, password):
        ...
        try:
            ...
            self.proxy_control_session.send(f'AUTHENTICATE "{self.password}"\r\n'.encode())
